...
def minhash_from_sparse_row(row_csr: csr_matrix, num_perm: int):
...
def topk_bruteforce_jaccard_indices(X_train: csr_matrix, train_nnz: np.ndarray, q: csr_matrix, k: int):
    q_nnz = q.nnz
    #inter = X_train.multiply(q).sum(axis=1).A1
    inter = (X_train @ q.T).toarray().ravel()
...
def topk_lsh_jaccard_indices(
    ...
    inter = (X_cand @ q.T).toarray().ravel()
...
def main():
    ...
    parser.add_argument("--perm", type=str, default="16,32,64", help="Comma-separated list of permutation counts (e.g. 16,32,64)")

    parser.add_argument("--tau",type=str,default="0.9,0.6,0.3", help="Comma-separated list of tau values (e.g. 0.7,0.5,0.4)")
    ...
    # =========================================================
    perm_list = [int(p) for p in args.perm.split(",")]
    tau_list = [float(t) for t in args.tau.split(",")]
    for num_perm in perm_list:
        for tau in tau_list:
            ...
            try:
                ...
                print(f"skipping perm={num_perm}, tau={tau} (Invalid Comb)")
                continue
            ...
            rows.append({
                ...
                "Parameters": f"Perm={num_perm}, tau={tau}"
            })
            print("tmp print\n------------------------------------")
            print(rows)
