...
# CONFIG
# =========================
TRAIN_PATH = "part1/joblibCache/dataTrain_cleaned.joblib"
TEST_PATH = "part1/joblibCache/dataTest_cleaned.joblib"
...
X_text_tr = train_data["Content"].fillna("").astype(str)
y_tr = train_data["Label"].values

# Test data has no labels; only content is used.
X_text_te = test_data["Content"].fillna("").astype(str)
...
# =========================
# Brute vs LSH (fixed train/test files)
# =========================
rows = []
...
print("\n================= Brute Force =================")
print("[Brute] Computing TRUE top-K neighbors...")
...
print("\n================= Results =================")
print(df[["Split", "Type", "BuildTime", "QueryTime", "TotalTime", "fraction_trueK_found", "Parameters"]]
      .to_string(index=False))

# Save to CSV for your report
df.to_csv("NEWfixedknn_lsh_train_test.csv", index=False)
print("\nSaved: NEWfixedknn_lsh_train_test.csv")
